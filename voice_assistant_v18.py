                ...
                while True:
                    ...
                    if not speech_started:
                        if volume_norm > SILENCE_THRESHOLD:
                            speech_started = True
                        # [수정] 청크 개수 대신 실제 경과 시간으로 타임아웃을 확인합니다.
...
def main():
    ...
    # [수정] 첫 인사말 재생 시 중복 출력을 방지합니다.
    # get_gemini_response 내부에서 이미 출력이 되므로 여기서는 출력하지 않습니다.
    ...
    try:
        was_last_loop_a_timeout = False # 연속 타임아웃 시 중복 요약을 방지하기 위한 플래그
        while True:
            if pixel_ring:
                pixel_ring.wakeup()
            ...
            if user_text:
                was_last_loop_a_timeout = False # 성공적인 음성 입력 시 플래그 초기화
                if pixel_ring:
                    pixel_ring.think()
                ...
                # 10초 타임아웃 발생 시
                if not was_last_loop_a_timeout:
                    print("10초 동안 음성 입력이 없어 대화 내용을 요약하고 다시 음성 입력을 기다립니다.")
                    summarize_conversation()
                    was_last_loop_a_timeout = True
                else:
                    print("계속 음성 입력을 기다립니다...")
