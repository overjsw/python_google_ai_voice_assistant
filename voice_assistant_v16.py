...
try:
    ...
    print("RPi.GPIO 라이브러리를 찾을 수 없거나, 이 시스템에서 지원되지 않습니다.")
    GPIO = None
...
def set_alsa_controls():
    """amixer를 사용하여 마이크의 자동 이득과 노이즈 억제 설정을 시도합니다."""
    # ReSpeaker 2-Mic HAT (WM8960)의 카드 번호는 일반적으로 '1'입니다.
    # 'arecord -l' 또는 'aplay -l' 명령어로 정확한 카드 번호를 확인할 수 있습니다.
    card_number = "1" 
    
    # 설정할 컨트롤과 값.
    # 참고: 'Noise Suppression'은 WM8960 코덱의 표준 amixer 컨트롤이 아닐 수 있습니다.
    # 이는 소프트웨어 처리(예: WebRTC VAD) 또는 특정 DSP 펌웨어에 의해 제공될 수 있습니다.
    # 이 스크립트는 해당 컨트롤이 존재한다고 가정하고 설정을 시도합니다.
    controls_to_set = {
        "'ALC Capture Target Volume'": "5",  # 자동 이득 제어 (Auto Gain Control) 목표 레벨
        "'Noise Suppression'": "2"           # 노이즈 억제 (Noise Suppression) 레벨
    }
    
    print("\nALSA 마이크 설정을 적용합니다...")
    all_successful = True
    
    # ALC 기능을 활성화하는 것이 필요할 수 있습니다.
    try:
        switch_command = ['amixer', '-c', card_number, 'sset', "'ALC Switch'", 'on']
        print(f"  > 실행: {' '.join(switch_command)}")
        subprocess.run(switch_command, check=True, capture_output=True, text=True)
    except Exception:
        # 이 스위치가 없거나 이미 켜져 있어도 괜찮습니다.
        print("  > 'ALC Switch'를 찾을 수 없거나 이미 활성화되어 있습니다.")

    for control, value in controls_to_set.items():
        try:
            command = ['amixer', '-c', card_number, 'sset', control, value]
            print(f"  > 실행: {' '.join(command)}")
            subprocess.run(command, check=True, capture_output=True, text=True)
        except subprocess.CalledProcessError:
            print(f"  [경고] '{control}' 설정에 실패했습니다. 해당 컨트롤이 시스템에 없을 수 있습니다.")
            all_successful = False
        except FileNotFoundError:
            print("  [오류] 'amixer' 명령어를 찾을 수 없습니다. 'sudo apt-get install alsa-utils'로 설치해주세요.")
            all_successful = False
            break
    
    if all_successful:
        print("마이크 설정이 성공적으로 적용되었습니다.")
    else:
        print("일부 마이크 설정 적용에 실패했습니다. 기본 설정으로 계속합니다.")
    print("-" * 20)
...
def main():
    ...
    if GPIO:
        ...
        # GPIO 17번 핀을 입력으로 설정하고, 내부 풀업 저항을 활성화합니다.
    ...
    if os.path.exists(SUMMARY_FILE):
        ...
        if summary_data and "summary" in summary_data:
            ...
            initial_prompt = get_gemini_response(context_prompt) # 첫 응답은 user 프롬프트 없이 바로 생성
        else:
            print("요약 파일을 읽는 데 실패했습니다. 새로운 대화를 시작합니다.")
    ...
    try:
        while True:
            ...
            if user_text:
                ...
                # 10초 타임아웃 발생 시
                print("10초 동안 음성 입력이 없어 대화 내용을 요약하고 다시 음성 입력을 기다립니다.")
                summarize_conversation()
                if pixel_ring:
                    pixel_ring.off()
                    time.sleep(1) # 잠시 멈춰 시각적 피드백 제공
