...
# int16 데이터 타입에 맞춘 임계값 (소음 환경에 따라 조절이 필요할 수 있습니다)
    ...
    try:
        ...
        with sd.InputStream(device=device_id, samplerate=SAMPLE_RATE, channels=CHANNELS, dtype='int16', blocksize=chunk_size) as stream:
            ...
            while True:
                ...
                # 디버깅을 위해 현재 볼륨 크기를 1초에 한 번 출력합니다.
                loop_count += 1
                if loop_count >= chunks_per_second:
                    print(f"현재 볼륨: {int(volume_norm):5d} (묵음 기준: {SILENCE_THRESHOLD})")
                    loop_count = 0
