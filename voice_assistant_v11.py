...
def record_audio_with_silence_detection(device_id):
    """음성 입력이 시작된 후에 묵음이 감지될 때까지 오디오를 녹음하고, 10초간 입력이 없으면 종료합니다."""
            ...
            while True:
                ...
                if not speech_started:
                    ...
                    if volume_norm > SILENCE_THRESHOLD:
                        print("음성 입력을 감지했습니다. 지금부터 묵음 감지를 시작합니다.")
                        speech_started = True
                    # 지정된 시간 동안 음성 입력이 없으면 타임아웃 처리
                    elif total_chunks_read > start_timeout_chunks:
                        print(f"{start_timeout_seconds}초 동안 음성 입력이 없어 녹음을 중단합니다.")
                        audio_data.clear() # 타임아웃 시 녹음 데이터 비우기
                        break
                else:
                    # 음성 입력이 시작된 후에는 묵음을 감지합니다.
                    if volume_norm < SILENCE_THRESHOLD:
    ...
    if not audio_data:
        # 이 경우는 타임아웃으로 인해 데이터가 비워진 경우입니다.
        print("녹음된 오디오 데이터가 없습니다 (타임아웃).")
        return False
...
def main():
    ...
    try:
        while True:
            ...
            # record_audio_with_silence_detection는 성공 시 True, 타임아웃이나 오류 시 False를 반환합니다.
            if record_audio_with_silence_detection(mic_device_id):
                ...
                if user_text:
                    ...
                    # 녹음은 성공했으나 STT 결과가 없는 경우 (소음 등)
                    print("음성을 인식하지 못했습니다. 다시 말씀해주세요.")
                    continue
            else:
                # 녹음 타임아웃 또는 오류 발생 시 다시 대기 상태로 돌아갑니다.
