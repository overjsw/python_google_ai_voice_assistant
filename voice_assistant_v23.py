...
def google_tts(text):
    ...
    try:
        ...
        # [수정] 존재하지 않는 'ko-KR-Studio-B' 대신 사용 가능한 고품질 음성 'ko-KR-Wavenet-A'로 변경
...
def summarize_conversation():
    ...
    try:
        ...
        summary_prompt = f"Please summarize the following entire conversation between 'Teacher Emily' and '{USER_NAME}' in Korean:\n\n{conversation_text}"
    ...
    try:
        ...
        while True:
            ...
            if summary_data and "summary" in summary_data:
                ...
                print("AI가 응답을 생성 중입니다...")
                try:
                    response = gemini_model.generate_content(conversation_history + [{'role': 'user', 'parts': [context_prompt]}])
                    initial_prompt = response.text
                except Exception as e:
                    print(f"Gemini API 오류: {e}")
                    initial_prompt = f"Hello {USER_NAME}! Let's talk again!"
            else:
                print("저장된 대화 요약이 없습니다. 새로운 대화를 시작합니다.")
                # 새로운 대화 시작 시, 히스토리를 시스템 프롬프트만 남기고 초기화
                conversation_history = [{'role': 'user', 'parts': [SYSTEM_PROMPT]}]
            ...
            while True:
                ...
                if user_text:
                    ...
                    if "END_CONVERSATION" in ai_text:
                        ...
                        break # inner loop -> starts a new session
